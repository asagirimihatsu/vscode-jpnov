 ...
 * `jpbook.*` command; `manage.ts` narrows on `kind`. `meta` carries one of the fixed
 * front-matter keys (META_KEYS) and its current value; `chapter` carries the document `line` the edit
 * planners key on.
 */
