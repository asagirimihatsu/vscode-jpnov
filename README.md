...
「Japanese Novel」は、無料のエディター「Visual Studio Code（VS Code）」を小説の執筆環境に変える拡張機能です。書いた文章はすぐ隣に縦書きで表示され、章をまとめて HTML・PDF・EPUB・テキストの「本」として出力できます。記法は[青空文庫の注記][AozoraAnno]そのまま。原稿はただのテキスト ファイルです。
...
単語辞書から「これは人名らしい」と推測して色を付けるような機能は、あえて持ちません。推測は外れるからです——ありふれた言葉が人名として塗られたり、作品の要になる造語が見過ごされたり。目立たせたい言葉は、[登場人物と造語のハイライト](#登場人物と造語のハイライト)に作者自身が登録します。[文章校正](#文章校正)も同じ考えで、単語辞書に基づくあいまいな指摘はしません。辞書を積み込まないぶん、動作も軽いままです。

### 入れたら、それだけで動きます

追加のソフトも、プログラムの知識も要りません。プレビューも本づくりも手元のパソコンの中だけで完結するので、オフラインでも使え、原稿がどこかへ送られることもありません（PDF 出力もブラウザーを借りるだけで、ネットにはつなぎません）。プレビューは入力のたびに、すぐ組み直されます。
...
8. **本にする** — 「本の一覧」の下にある「PDF に出力」を押すと、開いているフォルダー内の `dist` に印刷用の PDF ができあがります。本が増えたら、一覧に戻ってチェックを入れれば、まとめて出力できます。
...
- **巻ごとに 1 冊** — 長編は `第一巻.jpbook`・`第二巻.jpbook` と分ければ、最新巻だけを 1 つの PDF にして編集部に渡せます。
...
- **HTML に出力** — ページ組版の縦書き HTML。ブラウザーでそのまま開ける 1 ファイル完結の形式です。
...
どれを選んでも、HTML・PDF・EPUB の出力では em ダッシュ（—）として組まれます。日本語のフォントでは、em ダッシュどうしが切れ目なく 1 本につながります。
...
HTML と PDF には、ページ番号とヘッダー（ページ上部の題名）が入ります。これらは**本ごとの情報**なので、`.jpbook` の冒頭に書きます。同じフォルダーで第一巻と第二巻を作っても、ヘッダーをそれぞれ変えられます。章と章の間に入れる区切り記号（章区切り）も、同じ場所で指定します。
...
点検は、地の文・セリフ・ルビの読みを区別して行われます。文体の規則が働くのは地の文だけなので、会話は会話のまま書けます。単語辞書による推測もしないので、作品の固有名詞はそのまま通ります。

- **既定でオン** — 半角カナ、分解済みの文字（NFD）、Shift JIS にない文字（旧字体や異体字）、ゼロ幅スペース、不正な制御文字、ダッシュの揃え。コピー＆ペーストで紛れ込む「見えない事故」を防ぎます。
...
- **登場人物**は、主語として読める場所（「林は」「神木ちゃんが」など）だけ色が付きます。会話文（「」の中）には色が付きません。
- **造語**は地の文で太字になります。
