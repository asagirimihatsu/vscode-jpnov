...
「Japanese Novel」は、無料のエディター「Visual Studio Code（VS Code）」を小説の執筆環境に変える拡張機能です。書いた文章はすぐ隣に縦書きで表示され、章をまとめて HTML・PDF・EPUB・テキストの「本」として出力できます。記法は[青空文庫の注記][AozoraAnno]そのまま。原稿はただのテキスト ファイルで、特別な形式に囲い込まれることはありません。
...
- **本づくり** — 章をまとめて、縦書きの HTML・印刷用の PDF・電子書籍の EPUB・青空文庫形式のテキストに出力。
...
5. **本を作る** — 左端のアクティビティ バーに現れる「小説」（本のアイコン）を開き、「本の一覧」の「＋」（本を作成…）を押して、本の名前を入力します（末尾の `.jpbook` は最初から入っています）。`.jpbook` 1 つが本 1 冊です。作った本がそのまま開き、「本の情報」と「目次」が現れます。
6. **章を書く** — 「目次」の「新しい章…」を押すと、`.jpnov` が入った入力欄が出ます。その前に章の名前だけを入力してください（この `.jpnov` が「小説の章」の目印です）。作った章は目次に加わり、そのままエディターで開きます。あとは書くだけです。
7. **縦書きで確かめる** — 章を開いた状態で、エディター右上の「プレビューを横に開く」アイコンを押すと、エディターの横に縦書きの組み上がりが表示されます。書くそばから反映されます。
8. **本にする** — 「本の一覧」の下にある「PDF に出力」を押すと、フォルダー内の `dist` に印刷用の PDF ができあがります。本が増えたら、一覧に戻ってチェックを入れれば、まとめて出力できます。

![「本の一覧」で本を開いたところ。題名やヘッダーが並ぶ「本の情報」と、章の「目次」](docs/images/vscode-books-panel.png)
...
- 左側にルビを振るときは、設定「行送り」を 2 倍以上にします。狭いままだと、左ルビが隣の行に重なります。
- 見出しの注記を書いても、章が分かれるわけではありません。**章＝ 1 ファイル**が区切りです。
- 対応していない注記はメモ扱いになり、出力には現れません。書きかけの注記（`］` の閉じ忘れ）は波線でお知らせします。

## 自動字下げ

和文では、段落の先頭に全角スペースを 1 つ置きます。セリフなど、`「` や `『` で始まる行には置きません。この決まりに合わせて、スペースを自動で入れたり外したりします。
...
本にするには、アクティビティ バーの「小説」を開きます。出力したい本にチェックを入れて、一覧の下のボタンを押します（HTML と EPUB はアイコンのボタンです）。

- **PDF に出力** — 印刷用の PDF。用紙は A4（既定）か A6 を選べます。向きはページの形に合わせて自動で決まります（設定で固定もできます）。
...
出力先はフォルダー内の `dist` です（設定「小説 — 組版と出力」で変更できます）。章と本のファイルはフォルダー分けしても大丈夫で、並びはそのまま出力先に反映されます。

**ふだんの操作は「本の一覧」でできます** — 本を開くと「本の情報」と「目次」が現れます。章を新しく作るときは「新しい章…」を、すでにある章を加えるときは「章を追加…」を押します。並べ替えは、ドラッグか上下のボタンでできます。不要になった章は「×」で本から外せます（ファイルは残ります）。題名やヘッダーは、「本の情報」の各行をクリックすればその場で編集できます。下の出力ボタンは、開いている本だけを出力します。どの操作も `.jpbook` の中身をそのまま書き換えるので、手で書いた場合と完全に同じ結果になります。
...
> **PDF 出力には Google Chrome か Microsoft Edge が必要です**（Chromium・Brave でも可）。パソコンに入っていれば自動で見つけます。見つからないときは、代わりに HTML を出力してお知らせしますので、ブラウザーを入れるか、設定「小説 — 組版と出力」で場所を指定してください。

## 組版の設定

既定の組版は **40字×34行**。投稿原稿でよく使われる書式です。字数・行数・行送り・本文のフォントは設定「小説 — 組版と出力」で変えられます。行送りの既定は文字サイズの 1.5 倍、本文のフォントの既定は OS 付属の明朝体です。
...
二倍ダッシュは、切れ目なく 1 本につながります。

本文で使う文字は、設定「小説 — 文章校正」で選べます。ほかのダッシュ文字や奇数個の連続は指摘され、電球アイコンから直せます。

- em ダッシュ `—`（U+2014）： JIS によれば、こちらこそが本当の全角ダッシュです。だが、パソコンの文字組版や日本語入力システムなどの歴史的な経緯で、投稿サイトの「記号挿入」ではこちらを入力しないことが多いです。
- 常用ダッシュ `―`（U+2015）：英語から直訳で「水平線」となります。投稿サイトでよく使われる、つまり事実となっています。[青空文庫の記号](https://www.aozora.gr.jp/aozora-manual/index-input.html#symbol)もこちらを使えます。事実は何よりも大事で、既定にしています。
- 罫線 `─`（U+2500）：レアですが、使用されている作品もあります。

設定で選ぶのは、原稿で入力する文字です。HTML・PDF・EPUB の出力では、選んだダッシュを em ダッシュ（—）として組み、切れ目なく 1 本につなげます。
...
*罫線を「赤」、行番号をオン、行送りを 2 倍にした PDF 出力例*
...
- `title` — 本の題名。「本の一覧」の表示名と EPUB の題名になります。
...
どれも書かなくてかまいません（書いた分だけ既定から変わります）。編集中はキー名や値の候補が自動で出ます。同じ内容は、「本の一覧」で本を開けば「本の情報」からもクリックで編集できます。

## 文章校正

書いている間、原稿を静かに点検します。

点検は、地の文・セリフ・ルビの読みを区別して行われます。地の文向けの文体の規則がセリフの言い回しにまで適用されて、会話のたびに波線が付く、ということはありません。単語辞書による推測もしないので、作品の固有名詞が誤字扱いされることもありません。

- **既定でオン** — 半角カナ、壊れた文字（NFD）、JIS にない文字（旧字体や異体字）、ゼロ幅スペース、不正な制御文字。コピー＆ペーストで紛れ込む「見えない事故」を防ぎます。ダッシュを揃える点検も既定でオンです。
...
登場人物の名前と、作品だけの造語を登録しておくと、執筆中に目立つ表示になります。主語が省略されがちなので、「いま誰の場面か」を見失いにくくなります。
...
**Q. 以前の原稿や今の出力したテキスト、または青空文庫からダウンロードした `*.txt` を VS Code で開いたら、文字が「&#xFFFD;」だらけになりました。**
文章はそのまま入っています、ご安心ください。右下から開き直すと読めます。

1. ウィンドウ右下の「UTF-8」をクリックします。

   ![ウィンドウ右下のステータス バーにある「UTF-8」の表示](docs/images/vscode-encoding-statusbar.png)
...
テキスト出力の既定は、青空文庫と同じ Shift JIS という日本語専用のエンコード（文字集）です。国際的な UTF-8 で出力したいときは、設定「小説 — 組版と出力」で選べます。 Shift JIS の利用が不要な場合は `jpnov.lint.common.shiftJisSafe` を外して、 JIS に関する警告を抑制できます。
