...
7. **本のファイルを作る** — 章がいくつかできたら、`作品名.jpbook` というファイルを作り、**読む順に 1 行 1 ファイル**で章の名前を書きます。`.jpbook` 1 つが本 1 冊です。
   ...
   冒頭に `---` で挟んだブロックを置くと、題名や柱（ページ上部に入る巻名など）といった**その本自身の情報**も書けます（詳しくは[ページ番号と柱](#ページ番号と柱)）。
...
`.jpbook`（本のファイル）1 つが本 1 冊になります。アクティビティ バーの「小説」を開き、出力したい本にチェックを入れて、「本の一覧」の右上のボタンで出力します。
...
HTML と PDF には、ページ番号と柱（ページ上部の題名）が入ります。これらは**本ごとの情報**なので、設定画面ではなく `.jpbook` の冒頭に書きます——同じフォルダーで第一巻と第二巻を作っても、柱をそれぞれ変えられます。
...
- `header` — 柱（ページ上部の中央に入る文字）。書かなければ柱なし。
...
どれも書かなくてかまいません（書いた分だけ既定から変わります）。編集中はキー名や値の候補が自動で出ます。
...
**Q. 見出しや目次ページは作れますか？**
いまのところ対応していません。章＝ 1 ファイルで区切り、`.jpbook` の並び順がそのまま本の順序になります。
