...
既定の組版は **40字×34行**。字数・行数・行送り・本文のフォントは設定「小説 — 組版と出力」で変えられます。行送りの既定は文字サイズの 1.5 倍、本文のフォントの既定は OS 付属の明朝体です。
...
設定「小説 — 文章校正」で選べます。既定は常用ダッシュ `―`（U+2015）で、[青空文庫の記号](https://www.aozora.gr.jp/aozora-manual/index-input.html#symbol)と同じ文字です。em ダッシュ `—`（U+2014）と罫線 `─`（U+2500）も選べます。ほかのダッシュ文字や奇数個の連続は指摘され、電球アイコンから直せます。

どれを選んでも、HTML・PDF・EPUB の出力では em ダッシュ（—）として組まれます。日本語のフォントで、 em ダッシュは切れ目なく 1 本につながります。
