...
ページ組版の出力には、ページ番号とヘッダー（ページ上部のタイトル）が入ります。これらは**本ごとの情報**なので、`.jpbook` の冒頭に書きます。同じフォルダーで第一巻と第二巻を作っても、ヘッダーをそれぞれ変えられます。章と章の間に入れる区切り記号（章区切り）も、同じ場所で指定します。
...
出力すると、その本の値が入ります。プレビューには見本の「タイトル」「ペンネーム」「NaN」が出ます。同じ表紙ファイルを、何冊の本にも使えます。見出しや傍点を付けるときは、開始／終了型で挟みます（`［＃大見出し］［＃ここに「タイトル」の値を表示］［＃大見出し終わり］`）。
