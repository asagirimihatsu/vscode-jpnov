...
本文で使う文字は、設定「小説 — 文章校正」で選べます。既定は常用ダッシュ `―`（U+2015）で、[青空文庫の記号](https://www.aozora.gr.jp/aozora-manual/index-input.html#symbol)と同じ文字です。em ダッシュ `—`（U+2014）と罫線 `─`（U+2500）も選べます。ほかのダッシュ文字や奇数個の連続は指摘され、電球アイコンから直せます。

どれを選んでも、HTML・PDF・EPUB の出力では em ダッシュ（—）として組まれます。
