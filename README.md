...
7. **本を作る** — 章がいくつかできたら、左端のアクティビティ バーに現れる「小説」（本のアイコン）を開き、「本の一覧」の「＋」（本を作成）でファイル名を入力します。開いた詳細ページで、章を読む順に追加し、題名やヘッダー（ページ上部に入る巻名など）もその場で入力できます。`.jpbook` 1 つが本 1 冊です。

   中身はただのテキスト ファイルです。章のファイル名（開いているフォルダーからのパス）が読む順に 1 行ずつ並び、冒頭に `---` で挟んだブロックを置けば**その本自身の情報**も書けます。手で書いても同じです（詳しくは[ページ番号とヘッダー](#ページ番号とヘッダー)）。
...
8. **本にする** — 「本の一覧」で、出力したい本にチェックが入っているのを確かめて、一覧の下の「PDF に出力」を押せば、フォルダー内の `dist` に印刷用の PDF ができあがります。それぞれの本の詳細ページから、その本だけを個別に出力することもできます。
...
本にするには、アクティビティ バーの「小説」を開きます。「本の一覧」で出力したい本にチェックを入れて、一覧の下のボタンを押します。
