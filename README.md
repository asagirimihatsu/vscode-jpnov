...
7. **本のファイルを作る** — 章がいくつかできたら、`作品名.jpbook` というファイルを作り、**読む順に 1 行 1 ファイル**で章の名前を書きます（パスは、開いているフォルダーを起点に数えます）。`.jpbook` 1 つが本 1 冊です。次の手順で開く「本の一覧」の「＋」（本を作成）でも、題名を入力して章を選ぶだけで同じファイルを作れます。
...
8. **本にする** — 左端のアクティビティ バーに現れる「小説」（本のアイコン）を開くと、「本の一覧」に今の `.jpbook` が並んでいます。チェックが入っているのを確かめて、一覧の下の「PDF に出力」を押せば、フォルダー内の `dist` に印刷用の PDF ができあがります。それぞれの本の詳細ページから、その本だけを個別に出力することもできます。
